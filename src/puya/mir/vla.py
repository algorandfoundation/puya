...
class VariableLifetimeAnalysis:
    ...
    def is_dead_store(self, op: models.BaseOp) -> bool:
        return isinstance(
            op, models.AbstractStore
        ) and op.local_id not in self.get_live_out_variables(op)

    @cached_property
    def _op_lifetimes(self) -> dict[models.BaseOp, _OpLifetime]:
        logger.debug(
            f"performing variable lifetime analysis on MIR subroutine {self.subroutine.signature.name}"
        )
        data = _initial_op_lifetimes(self.subroutine)
...
def _initial_op_lifetimes(sub: models.MemorySubroutine) -> dict[models.BaseOp, _OpLifetime]:
    result = dict[models.BaseOp, _OpLifetime]()
    block_map = {b.block_name: b.ops[0] for b in sub.body}
