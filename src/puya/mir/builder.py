...
def build_new_slot_sub(allocation_slot: int) -> models.MemorySubroutine:
    # TODO: having this in IR may allow for inlining...
    """
    load SLOT       // slot
    bitlen          // free_slot
    ...
    local_ids = dict[str, AVMType]()

    def _local_id(var: str) -> str:
        return f"{var}#0"

    def load(var: str) -> models.AbstractLoad:
        local_id = _local_id(var)
        return models.AbstractLoad(
            local_id=local_id,
            atype=local_ids[local_id],
            source_location=None,
        )

    def store(var: str, atype: AVMType) -> models.AbstractStore:
        local_id = _local_id(var)
        try:
            assert local_ids[local_id] == atype
        except KeyError:
            local_ids[local_id] = atype

        return models.AbstractStore(
            local_id=local_id,
            atype=atype,
            source_location=None,
        )

    slot_allocations = make_op(AVMOp.load, allocation_slot, local_id="slot_allocations")
        ...
        body=[
            models.MemoryBasicBlock(
                ...
                mem_ops=[
                    ...
                    store("free_slot", AVMType.uint64),
                    slot_allocations,
                    models.Int(
                        value=256,
                        source_location=None,
