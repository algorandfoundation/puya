...
def x_stack_allocation(_ctx: ProgramMIRContext, sub: mir.MemorySubroutine) -> None:
    ...
    edge_sets = _get_edge_sets(sub, vla)
    if not edge_sets:
        # nothing to do
        return

    logger.debug(f"Found {len(edge_sets)} edge set/s for {sub.signature.name}")
    _schedule_sets(vla, edge_sets)

    if not _validate_x_stacks(edge_sets):
        raise InternalError("Could not schedule x-stack")

    _add_x_stack_ops_to_edge_sets(sub, edge_sets)
...
class _BlockRecord:
    ...
    live_in: Set[str]
    live_out: Set[str]
    children: "list[_BlockRecord]" = attrs.field(factory=list)
    parents: "list[_BlockRecord]" = attrs.field(factory=list)
    co_parents: "list[_BlockRecord]" = attrs.field(factory=list)
    siblings: "list[_BlockRecord]" = attrs.field(factory=list)

    x_stack_in: Sequence[str] | None = None
    x_stack_out: Sequence[str] | None = None

    def __repr__(self) -> str:
        # due to recursive nature of _BlockRecord, provide str implementation to
        # simplify output
        return f"_BlockRecord({self.block})"
...
@attrs.frozen
class _EdgeSet:
    out_blocks: Sequence[_BlockRecord] = attrs.field(converter=tuple[_BlockRecord, ...])
    in_blocks: Sequence[_BlockRecord] = attrs.field(converter=tuple[_BlockRecord, ...])


def _get_edge_sets(sub: mir.MemorySubroutine, vla: VariableLifetimeAnalysis) -> Sequence[_EdgeSet]:
    records = {
        block.block_name: _BlockRecord(
            block=block,
            local_references=[
    ...
    edge_sets = dict[_EdgeSet, None]()
    for block in blocks:
        # keep expanding edge set until it stabilizes
        edge_set = _get_edge_set(block)
        if edge_set:
...
def _get_edge_set(block: _BlockRecord) -> _EdgeSet | None:
    ...
    return _EdgeSet(out_blocks, in_blocks) if in_blocks else None
...
def _schedule_sets(vla: VariableLifetimeAnalysis, edge_sets: Sequence[_EdgeSet]) -> None:
    ...
    for edge_set in edge_sets:
        ...
        x_stack_candidates = [
            *(_sort_by_appearance(common_locals, b.block, load=False) for b in out_blocks),
            *(_sort_by_appearance(common_locals, b.block, load=True) for b in in_blocks),
        ]

        # find an x_stack for this EdgeSet
        x_stack = _find_shared_x_stack(x_stack_candidates)
...
def _find_shared_x_stack(x_stack_candidates: Sequence[Sequence[str]]) -> Sequence[str]:
    ...
    def lcs(s1: tuple[str, ...], s2: tuple[str, ...]) -> tuple[str, ...]:
        ...
        if result is None:
            ...
            if i == 0 or j == 0:
                ...
                result = max(lcs(s1[:-1], s2), lcs(s1, s2[:-1]), key=_len_and_value)
            cache[key] = result
        return result

    shared, *others = sorted({tuple(s) for s in x_stack_candidates}, key=_len_and_value)
    for other in others:
...
def _validate_x_stacks(edge_sets: Sequence[_EdgeSet]) -> bool:
    ok = True
    for edge_set in edge_sets:
        for parent in edge_set.out_blocks:
            for child in edge_set.in_blocks:
                ok = _validate_pair(parent, child) and ok
    return ok


def _validate_pair(parent: _BlockRecord, child: _BlockRecord) -> bool:
...
def _add_x_stack_ops_to_edge_sets(
    sub: mir.MemorySubroutine, edge_sets: Sequence[_EdgeSet]
) -> None:
    ...
    for record in records:
        ...
        _add_x_stack_ops(sub, record)


def _add_x_stack_ops(sub: mir.MemorySubroutine, record: _BlockRecord) -> None:
    block = record.block
    # determine ops to replace
    load_ops = _get_x_stack_load_ops(record)
    store_ops = _get_x_stack_store_ops(record)
...
def _get_x_stack_load_ops(record: _BlockRecord) -> set[mir.AbstractLoad]:
    block = record.block
    assert block.x_stack_in is not None
...
def _get_x_stack_store_ops(record: _BlockRecord) -> set[mir.AbstractStore]:
    block = record.block
    assert block.x_stack_out is not None
