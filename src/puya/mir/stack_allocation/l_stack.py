...
def _koopmans(sub: mir.MemorySubroutine) -> None:
...
class _UsagePair:
    ...
    @staticmethod
    def by_distance(pair: "_UsagePair") -> tuple[int, int, int]:
        return pair.b_index - pair.a_index, pair.a_index, pair.b_index


def _find_usage_pairs(block: mir.MemoryBasicBlock) -> list[_UsagePair]:
    ...
    for uses in variables.values():
        ...
        for (a_index, a), (b_index, b) in itertools.pairwise(uses):
            ...
            pairs.append(_UsagePair(a=a, b=b, a_index=a_index, b_index=b_index))

    return sorted(pairs, key=_UsagePair.by_distance)


def _copy_usage_pairs(
    sub: mir.MemorySubroutine, block: mir.MemoryBasicBlock, pairs: list[_UsagePair]
) -> None:
    ...
    for block in sub.body:
        ...
        while op_idx < len(ops) - 1:
            ...
            if isinstance(a, mir.StoreLStack):
                ...
                a = attrs.evolve(a, copy=True, produces=(f"{a.local_id} (copy)",))
                ops[window] = [a]
            op_idx += 1
    ...
    for block in sub.body:
        ...
        while op_idx < len(ops):
            ...
            if producer.produces:
                maybe_remove_window = slice(next_op_idx, next_op_idx + len(producer.produces))
                maybe_remove = ops[maybe_remove_window]
                # if they all match then this means all values are implicitly on the l-stack
                # and we can safely remove the store ops
                if len(maybe_remove) == len(producer.produces) and all(
                    (
                        isinstance(op, mir.StoreLStack)
                        and not op.copy
                        and op.local_id in producer.produces
                    )
                    for op in maybe_remove
                ):
