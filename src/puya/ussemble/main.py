...
def assemble_program(
    ...
    return AssembledProgram(
        ...
        source_map=_get_source_map(assembled.source_map),
    )


def _get_source_map(source_map: Mapping[int, models.Node]) -> bytes:
    mappings = []
    ...
    last_op = next(iter(source_map.values()))
    ...
    return json.dumps(debug_map, indent=True).encode("utf-8")
