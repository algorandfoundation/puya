...
def create_server(thread_count: int = 4) -> JsonRPCServer:
    ...
    def analyse(params: AnalyseParams) -> AnalyseResult:
        ...
        try:
            with (
                ...
                def run_with_cancellation_checks() -> bool:
                    if message_registry.is_cancelled(message_id):
                        log_ctx.logs.append(
                            Log(
                                level=LogLevel.warning,
                    ...
                    return message_registry.is_cancelled(message_id)

                was_cancelled = run_with_cancellation_checks()
...
def main() -> None:
    ...
    parser.add_argument("--threads", type=int, default=4, help="Worker thread count")
