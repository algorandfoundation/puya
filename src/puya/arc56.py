...
def _method_actions(method: ARC4BareMethod | ARC4ABIMethod) -> models.MethodActions:
    ...
    return models.MethodActions(
        create=[
            ...
            if config.create != ARC4CreateOption.disallow and _allowed_create_oca(oca.name)
        ],
        call=[
            oca.name
            for oca in config.allowed_completion_types
            if config.create != ARC4CreateOption.require and _allowed_call_oca(oca.name)
        ],
    )
