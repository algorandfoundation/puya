...
class AppStateClassExpressionBuilder(IntermediateExpressionBuilder):
    ...
    def call(
        ...
        if self._storage is not None and self._storage != storage_wtype:
            raise CodeError(
                "App global state explicit type annotation does not match first argument"
...
class AppStateExpressionBuilder(IntermediateExpressionBuilder):
    ...
    def lvalue(self) -> Lvalue:
        return AppStateExpression.from_state_def(self.state_def, self.source_location)

    def member_access(self, name: str, location: SourceLocation) -> ExpressionBuilder | Literal:
        match name:
            case "exists":
                return AppStateExistsExpressionBuilder(
            ...
            case "put":
                return AppStatePutExpressionBuilder(
                    state_def=self.state_def,
                    location=self.source_location,
                    key_expression=self.key_expression,
                )
            ...
            case "delete":
                return AppStateDeleteExpressionBuilder(
                    state_def=self.state_def,
                    location=self.source_location,
                    key_expression=self.key_expression,
                )
...
class AppStateExistsExpressionBuilder(AppStateMethodBaseExpressionBuilder):
    def call(
        self,
        args: Sequence[ExpressionBuilder | Literal],
        arg_kinds: list[mypy.nodes.ArgKind],
...
class AppStateDeleteExpressionBuilder(AppStateMethodBaseExpressionBuilder):
    def call(
        self,
        args: Sequence[ExpressionBuilder | Literal],
        arg_kinds: list[mypy.nodes.ArgKind],
...
class AppStatePutExpressionBuilder(AppStateMethodBaseExpressionBuilder):
    def call(
        self,
        args: Sequence[ExpressionBuilder | Literal],
        arg_kinds: list[mypy.nodes.ArgKind],
