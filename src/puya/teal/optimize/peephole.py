...
def peephole(block: models.TealBlock, opt_level: int) -> bool:
    ...
    while start_idx < len(result):
        ...
        # note: optimizing pair and singles removes a lot of ops that can bloat
        #       program size, this is why they are done even at O0
...
def _optimize_pair(a: models.TealOp, b: models.TealOp) -> tuple[list[models.TealOp], bool]:
    ...
    match a, b:
        ...
        case models.TealOp(op_code="dup" | "assert") as eliminate_op, models.Return() as ret_op:
            return [
                attrs.evolve(
                    ret_op,
                    error_message=ret_op.error_message or eliminate_op.error_message,
                    source_location=ret_op.source_location or eliminate_op.source_location,
                )
            ], True
    return [a, b], False
