...
class ARC4Encode(Expression):
    ...
    value: Expression
    wtype: wtypes.ARC4Type | wtypes.BytesWType = attrs.field(default=wtypes.bytes_wtype)
    """When it is `bytes_wtype`, encoding is derived from `value` expression"""
...
class NamedTupleExpression(Expression):
    ...
    def _validate_values(self, _instance: object, values: Mapping[str, Expression]) -> None:
        ...
        for field_name, field_value in self.values.items():
            expected_wtype = self.wtype.fields[field_name].type
...
class FieldExpression(Expression):
    ...
    def _wtype_factory(self) -> WType:
        ...
        try:
            return dataclass_type.fields[self.name].type
        except KeyError:
...
class NewStruct(Expression):
    ...
    def _validate_values(self, _instance: object, values: Mapping[str, Expression]) -> None:
        ...
        for field_name, field_value in self.values.items():
            ...
            if field_value.wtype != expected_wtype.type:
                raise CodeError("Invalid argument type(s)", self.source_location)
