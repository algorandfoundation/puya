...
class _Problem:
    message: str
    severity: typing.Literal[LogLevel.error, LogLevel.warning] = attrs.field(
        default=LogLevel.warning
    )


Checker = Callable[[models.Intrinsic], _Problem | None]
    ...
    def visit_intrinsic_op(self, intrinsic: models.Intrinsic) -> None:
        for checker in (_check_signature_constrained_args, *_CHECKERS.get(intrinsic.op, ())):
            if problem := checker(intrinsic):
                logger.log(
                    problem.severity,
                    f"{problem.message}; will fail at runtime if reached",
...
def _make_uint64_result_check(
    ...
    def check(intrinsic: models.Intrinsic) -> _Problem | None:
        match intrinsic.args:
            case [models.UInt64Constant(value=a), models.UInt64Constant(value=b)]:
                result = reducer(a, b)
                if result > algo_constants.MAX_UINT64:
                    return _Problem("uint64 constant overflow")
                if result < 0:
                    return _Problem("uint64 constant underflow")
        return None
        ...
        match intrinsic.args:
            case [bytes_arg, models.UInt64Constant(value=offset)]:
                if offset + byte_size > _bytes_length_or_max(bytes_arg):
                    return _Problem(f"{intrinsic.op.code} buffer over-read")
        return None
...
def _check_get_or_set_bit_index(intrinsic: models.Intrinsic) -> _Problem | None:
    match intrinsic.args:
        case [arg, models.UInt64Constant(value=index), *_]:
            match arg.ir_type.avm_type:
                case AVMType.uint64:
                    if index >= 64:
                        return _Problem("bit index of uint64 is out of bounds")
                case AVMType.bytes | AVMType.any:
                    if index >= (8 * _bytes_length_or_max(arg)):
                        return _Problem("bit index of bytes is out of bounds")
    return None


def _check_get_or_set_byte_index(intrinsic: models.Intrinsic) -> _Problem | None:
    match intrinsic.args:
        case [bytes_arg, models.UInt64Constant(value=index), *_]:
            if index >= _bytes_length_or_max(bytes_arg):
                return _Problem("byte index of bytes is out of bounds")
    return None


def _check_signature_constrained_args(intrinsic: models.Intrinsic) -> _Problem | None:
        ...
        match expected_type:
            case types.PrimitiveIRType.state_key:
                ...
                if (
                    ...
                    return _Problem("state key constant exceeds 64 bytes", LogLevel.error)
            case types.PrimitiveIRType.box_key:
                ...
                if bytes_value is not None and not (
                    ...
                    return _Problem(
                        "box key constant has invalid length (must be 1-64 bytes)", LogLevel.error
                    )
            case types.PrimitiveIRType.biguint:
                ...
                if (
                    ...
                    return _Problem("biguint constant operand exceeds 512 bits")
    return None
...
def _check_exp_undefined(intrinsic: models.Intrinsic) -> _Problem | None:
    match intrinsic.args:
        case [models.UInt64Constant(value=a), models.UInt64Constant(value=b)]:
            if a == 0 and b == 0:
                return _Problem("uint64 exp result is undefined")
    return None


def _check_extract(intrinsic: models.Intrinsic) -> _Problem | None:
    ...
    if start > max_len:
        return _Problem("start index for extract is out of bounds")
    if length != 0 and start + length > max_len:
        return _Problem("end index for extract is out of bounds")
    return None


def _check_extract3(intrinsic: models.Intrinsic) -> _Problem | None:
    match intrinsic.args:
        case [
            arg,
            ...
            if start + length > _bytes_length_or_max(arg):
                return _Problem("extract3 buffer overflow")
    return None


def _check_substring(intrinsic: models.Intrinsic) -> _Problem | None:
    ...
    if end > _bytes_length_or_max(arg):
        return _Problem("substring buffer overflow")
    return None


def _check_substring3(intrinsic: models.Intrinsic) -> _Problem | None:
    ...
    if isinstance(start_val, models.UInt64Constant):
        ...
        if start > max_arg_len:
            return _Problem("substring3 buffer over-read")
    end = None
    ...
    if start is not None and end is not None and end < start:
        return _Problem("substring3 has end preceding start")
    return None


def _check_replace2(intrinsic: models.Intrinsic) -> _Problem | None:
    ...
    if start + len(replacement) > _bytes_length_or_max(arg):
        return _Problem("replace2 buffer overflow")
    return None


def _check_replace3(intrinsic: models.Intrinsic) -> _Problem | None:
    match intrinsic.args:
        case [arg, models.UInt64Constant(value=start), replacement_arg]:
            ...
            if replacement is not None and start + len(replacement) > _bytes_length_or_max(arg):
                return _Problem("replace3 buffer overflow")
    return None


def _check_concat(intrinsic: models.Intrinsic) -> _Problem | None:
    ...
    if a is not None and b is not None and len(a) + len(b) > algo_constants.MAX_BYTES_LENGTH:
        return _Problem("concat buffer overflow")
    return None


def _check_biguint_sub_underflow(intrinsic: models.Intrinsic) -> _Problem | None:
    a, b = intrinsic.args
    ...
    if a_const is not None and b_const is not None and a_const - b_const < 0:
        return _Problem("biguint constant underflow")
    return None
...
def _check_txn_group_index_arg(intrinsic: models.Intrinsic) -> _Problem | None:
    match intrinsic.args[0]:
        case models.UInt64Constant(value=value):
            if value >= algo_constants.MAX_TRANSACTION_GROUP_SIZE:
                return _Problem("txn group index constant exceeds maximum group size")
    return None


def _check_txn_group_index_immediate(intrinsic: models.Intrinsic) -> _Problem | None:
    t = intrinsic.immediates[0]
    assert isinstance(t, int)
    if t >= algo_constants.MAX_TRANSACTION_GROUP_SIZE:
        return _Problem("txn group index immediate exceeds maximum group size")
    return None
