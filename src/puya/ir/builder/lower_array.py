...
class ArrayNodeReplacer(IRMutator):
    ...
    def _read_item(
        ...
        return decode_array_item(self.context, item_bytes, element_type, loc)
...
def encode_array_item(
    ...
    for value, sub_type in zip(values, EncodedTupleType.expand_types(element_type), strict=True):
        if sub_type == PrimitiveIRType.uint64:
            value = factory.itob(value, "sub_item", return_type=array_type)
        encoded = factory.concat(encoded, value, "encoded", return_type=array_type)
    return encoded
