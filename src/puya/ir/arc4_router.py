...
def _check_allowed_oca(
    ...
    match allowed_ocas, not_allowed_ocas:
        ...
        case _:
            condition = _bit_and(
                _left_shift(_on_completion(location), location),
                _bit_packed_oca(allowed_ocas, location),
                location,
            )
...
def _check_allowed_oca_and_create(method: md.ARC4Method) -> Sequence[awst_nodes.Statement]:
    ...
    if not not_allowed_ocas:
        ...
        if len(allowed_ocas) > 1:
            oca_desc = f"one of {oca_desc}"
        error_message = f"OnCompletion must be {oca_desc}"
        match allowed_ocas, not_allowed_ocas:
            case [[single_allowed], _]:
                conditions.append(
                    (
                        error_message,
                        awst_nodes.NumericComparisonExpression(
                            lhs=_on_completion(location),
                            rhs=awst_nodes.UInt64Constant(
                                source_location=location,
                                value=single_allowed.value,
    ...
    match method.create:
        case awst_nodes.ARC4CreateOption.allow:
            ...
            pass
        case awst_nodes.ARC4CreateOption.disallow:
            conditions.append(("can only call when not creating", _non_zero(app_id)))
        case awst_nodes.ARC4CreateOption.require:
            conditions.append(("can only call when creating", _is_zero(app_id)))
        case invalid:
            typing.assert_never(invalid)
    ...
    assert_statement = awst_nodes.ExpressionStatement(
        awst_nodes.AssertExpression(
            condition=condition,
            error_message=error_message,
            source_location=location,
...
def _build_abi_wrapper(
    ...
    wrapper_method = awst_nodes.Subroutine(
        ...
        body=awst_nodes.Block(
            body=[call_and_maybe_log, _return(True, abi_loc)],
...
def _route_abi_methods(
    ...
    if not no_op_by_create:
        ...
        no_op_routing = (
            *_check_allowed_oca([OnCompletionAction.NoOp], router_location),  # TODO: simplify
            *_create_abi_switch(
                router_location,
...
def create_abi_router(
    ...
    approval_program = awst_nodes.ContractMethod(
        ...
        body=_create_block(router_location, None, *router, _return(False, router_location)),
        documentation=awst_nodes.MethodDocumentation(),
