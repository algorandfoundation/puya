...
def repeated_extends_simplification(_: CompileContext, subroutine: models.Subroutine) -> bool:
    ...
    for block in subroutine.body:
        for op in block.ops:
            if isinstance(op, models.Assignment) and isinstance(op.source, models.ArrayExtend):
    ...
    for current_extends in extends.values():
        ...
        if not maybe_previous_extends:
            continue
        maybe_previous_const_values = get_byte_constant(subroutine, maybe_previous_extends.values)
        maybe_current_const_values = get_byte_constant(subroutine, current_extends.values)
        if maybe_previous_const_values and maybe_current_const_values:
            ...
            current_extends.values = models.BytesConstant(
                ...
                ir_type=maybe_current_const_values.ir_type,
            )
            modified = True
    return modified
