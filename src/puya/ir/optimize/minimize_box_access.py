...
from puya.ir.encodings import Encoding, FixedArrayEncoding, TupleEncoding
...
class _AggregateReplacer(MutatingRegisterContext):
    ...
    def visit_aggregate_read_index(self, read: models.AggregateReadIndex) -> models.ValueProvider:
        agg_encoding = read.aggregate_encoding
        if not agg_encoding.is_fixed:
            return read
...
def _combine_box_and_aggregate_read(
    ...
    if agg_read.element_encoding.is_bit:
        ...
        if len(agg_read.indexes) == 1:
            ...
            assert box_extract is not None, "box_extract should not be None"
    ...
    for i, index in enumerate(indexes):
        if isinstance(base_encoding, TupleEncoding) and isinstance(index, int):
            ...
            if check_array_bounds:
                # exit loop if value can fit on stack instead of asserting,
                # since the box_extract will ensure the array has no trailing data
