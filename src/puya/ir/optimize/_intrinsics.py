...
This module holds the bits that more than one pass needs: the op-category frozensets
(consumed by dead-code-elimination and inlining), the byte-size table for the
`extract_uintN` ops, the small `valid_uint64` predicate, the bytes-encoding helpers, and
the boolean-context one-const simplifier used by `intrinsic_simplification`. The per-op
constant-folding logic lives with its sole consumer in `global_value_numbering`.
"""
...
def simplify_uint64_binary_op_one_const(
    ...
    """Algebraic simplification of `op(a, b)` in a *boolean context* when at least
    one operand is constant.

    Returns an int for a folded literal, LEFT/RIGHT for a pass-through to operand
    a/b, or None if no rule fires. Because the result is only consumed for its
    truthiness, every operand is treated as bool-safe, so e.g. `a >= 1` folds to
    `a`. Does NOT handle `op == AVMOp.eq` — the caller emits its own `!operand`
    rewrite (the rewrite shape is caller-specific).

    GVN has a sibling value-context variant that does not assume bool-safety; see
    `global_value_numbering._simplify_uint64_binary_op_one_const`.
    """
    match op:
        case AVMOp.gte:
            # a >= 0 <-> 1
