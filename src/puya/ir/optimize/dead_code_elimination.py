...
from puya.ir._utils import IRSubroutineCollector, bfs_block_order
...
def remove_unused_subroutines(program: models.Program) -> bool:
    subroutines = IRSubroutineCollector.collect(program.subroutines, start=program.main)
    if modified := (len(subroutines) != len(program.subroutines)):
