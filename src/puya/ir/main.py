...
from puya.utils import StableSet, attrs_extend, set_remove, unique
...
class FoldedContract:
    ...
    structs: list[wtypes.ARC4Struct] = attrs.field(factory=list)
...
def _fold_state_and_special_methods(
    ...
    for c in [contract, *bases]:
        ...
        for state in c.app_state.values():
            state_mapping: dict[str, ContractState]
            match state.kind:
                case awst_nodes.AppStorageKind.app_global:
                    state_mapping = result.global_state
                case awst_nodes.AppStorageKind.account_local:
                    state_mapping = result.local_state
                case awst_nodes.AppStorageKind.box:
                    state_mapping = result.boxes
                case _:
                    typing.assert_never(state.kind)
            if state.key_wtype and isinstance(state.key_wtype, wtypes.ARC4Struct):
                struct_types.append(state.key_wtype)
            if isinstance(state.storage_wtype, wtypes.ARC4Struct):
                struct_types.append(state.storage_wtype)
            state_mapping[state.member_name] = _get_contract_state(state)

    arc4_method_refs = _gather_arc4_methods(ctx, contract)
    for arc4_method in arc4_method_refs:
        struct_types.extend(
            [arg.wtype for arg in arc4_method.args if isinstance(arg.wtype, wtypes.ARC4Struct)]
        )
        if isinstance(arc4_method.return_type, wtypes.ARC4Struct):
            struct_types.append(arc4_method.return_type)
...
def _wtypes_to_structs(structs: Sequence[wtypes.ARC4Struct]) -> dict[str, ARC4Struct]:
    """
    ...
    struct_results = dict[wtypes.ARC4Struct, ARC4Struct]()
    while structs:
        struct = structs.pop()
...
def _wtype_to_struct(struct: wtypes.ARC4Struct) -> ARC4Struct:
    fields = []
    for field_name, field_wtype in struct.fields.items():
        fields.append(
            ARC4StructField(
                ...
                struct=field_wtype.name if isinstance(field_wtype, wtypes.ARC4Struct) else None,
            )
        )
    return ARC4Struct(
