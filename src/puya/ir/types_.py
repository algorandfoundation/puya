...
def wtype_to_abi_name(
    ...
    visitor = _WTypeToABIName(loc, resource_encoding)
...
class _WTypeToABIName(WTypeVisitor[str]):
    ...
    def visit_basic_type(self, wtype: wtypes.WType) -> str:
        if wtype == wtypes.biguint_wtype:
            return "uint512"
        elif wtype == wtypes.uint64_wtype:
            return "uint64"
        elif wtype == wtypes.bool_wtype:
            return "bool"
        elif wtype == wtypes.account_wtype:
            if self.resource_encoding == "index":
                return "account"
            else:
                return "address"
        elif wtype == wtypes.application_wtype:
            if self.resource_encoding == "index":
                return "application"
            else:
                return "uint64"
        elif wtype == wtypes.asset_wtype:
            if self.resource_encoding == "index":
                return "asset"
            else:
                return "uint64"
