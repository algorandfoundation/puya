...
class CompileParams:
    ...
    """absolute paths mapped to their lines"""
...
def create_server(max_workers: int | None) -> JsonRPCServer:
    ...
    def analyse(params: AnalyseParams) -> AnalyseResult:
        ...
        with (
            ...
            log_exceptions(),
            _disable_gc(),  # temporarily disable gc while analysing
            pushd(dummy_path),
        ):
            awst = params.awst
            compilation_set = {
...
def _with_shutdown[**P, T](server: JsonRPCServer, f: Callable[P, T]) -> Callable[P, T]:
    """triggers a shutdown after running a handler, used for --single-request option"""
