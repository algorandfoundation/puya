...
from wyvern.awst_build.eb.named_int_constants import NamedIntegerConstsTypeBuilder
...
class FunctionASTConverter(
    ...
    def _visit_ref_expr_of_algopy(
        ...
        if fullname.startswith(constants.ALGOPY_GEN_PREFIX):
        ...
        match analyzed:
            ...
            case mypy.nodes.RevealExpr(expr=mypy.nodes.Expression() as inner_expr):
                ...
                if isinstance(result, Literal):
                    self.context.note(f"algopy node is literal of {result.value!r}", call)
                else:
                    try:
                        the_value = result.rvalue()
                    except WyvernError:
                        self.context.note(f"algopy node is {result!r}", call)
                    else:
                        self.context.note(f'algopy type is "{the_value.wtype.name}"', call)
                return result
            case _:
                raise CodeError(
