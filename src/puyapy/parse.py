...
MAX_SUPPORTED_ALGOPY_VERSION_EX = version.parse("3.6.0")
...
def parse_python(
    ...
    *,
    ...
    """Parse the build sources and resolve all imported modules (recursively)"""
...
def fast_to_awst(module_data: Mapping[str, "_ModuleData"]) -> ParseResult:
    ...
    for module_name, mypy_module_state in sorted_mypy_modules.items():
        ...
        try:
            ...
            continue
        assert mypy_module.fullname == md.module, "mypy and parsed module name mismatch"
...
def _fast_parse_and_resolve_imports(
    ...
    *,
    ...
    while source_queue:
        ...
        parsed = source_provider.parse_source(
            module_path=rs.path,
