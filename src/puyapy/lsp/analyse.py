...
class CodeAnalyticTracker:
    ...
    async def parse(self, uris: Sequence[str]) -> Mapping[str, Sequence[Log]]:
        ...
        uri_logs = {p.as_uri(): list[Log]() for p in paths}
    ...
    async def _parse_and_log(self, puyapy_options: PuyaPyOptions) -> Sequence[Log]:
        ...
        try:
            ...
            return []
        self._update_dependencies(parse_result)
        logs = list(parse_ctx.logs)
        # lower Typed AST to AWST
        if not parse_ctx.num_errors:
            with logging_context(output_log=False) as awst_ctx, log_exceptions():
    ...
    def _update_dependencies(self, parse_result: ParseResult) -> None:
        ...
        for module_id, module in modules.items():
            ...
            for import_ in module.node.imports:
                if isinstance(import_, mypy.nodes.Import):
                    ...
                    # include any known submodules
                    sub_module_pattern = re.compile(f"{module_id}\\.\\w+$")
