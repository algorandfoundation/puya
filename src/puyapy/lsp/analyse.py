...
from puya.utils import StableSet, get_cwd, set_cwd
...
class DocumentAnalysis:
    ...
    symbols: dict[str, "_AlgopyImport"]
    """known algopy symbols imported in document"""
    logs: list[Log]
    """warning or errors present in document"""
    fixes: list[CodeFix]
    """possible fixes suggested in document"""

    def get_alias_and_import_insert(self, full_name: str) -> str | SymbolWithImport | None:
        ...
        try:
            ...
            assert isinstance(import_symbol, _ImportSymbol), "expected a symbol"
        ...
        try:
            ...
            if isinstance(module_alias, _ImportAll):
                return symbol
            # if module itself was imported then can use that
            elif isinstance(module_alias, _ImportedModule):
                return f"{module_alias.alias}.{symbol}"
            # if just siblings of the parent module were imported then use symbol
            # and provide a location to insert a new import statement
            elif isinstance(module_alias, _ImportedFrom):
                return SymbolWithImport(
                    alias=symbol,
                    import_stmt=f"from {symbol_module} import {symbol}",
                    import_line=module_alias.insert_at.line,
                )
        return None
...
class _ImportedModule:
    """
    Module imported with or without an alias

    e.g `import algopy.arc4 as bar` or `import algopy.arc4`
    """
...
class _ImportSymbol:
    """
    Indicates a symbol that was imported directly from a module

    e.g. `from algopy import UInt64 as Foo` or `from algopy import UInt64`
    """

    alias: str


_AlgopyImport = _ImportedModule | _ImportedFrom | _ImportAll | _ImportSymbol
...
class CodeAnalyser:
    ...
    ) -> dict[Path, tuple[dict[str, _AlgopyImport], list[CodeFix], list[Log]]] | None:
        # parse Typed AST
    ...
    def _analyse_imports(self, parse_result: ParseResult) -> dict[Path, dict[str, _AlgopyImport]]:
        ...
        path_algopy_symbols = dict[Path, dict[str, _AlgopyImport]]()
                ...
                if isinstance(import_, mypy.nodes.Import):
                    for id_, alias in import_.ids:
                        ...
                        if id_.startswith("algopy.") or id_ == "algopy":
                            symbols[id_] = _ImportedModule(alias=alias or id_)
                    ...
                    symbols[import_module_id] = _ImportedFrom(
                    ...
                    for name, alias in import_.names:
                        ...
                        if maybe_sub_module_id in modules:
                            ...
                            symbols[maybe_sub_module_id] = _ImportedModule(alias=alias or name)
                        else:
                            symbols[maybe_sub_module_id] = _ImportSymbol(alias=alias or name)
                elif isinstance(import_, mypy.nodes.ImportAll):
                    import_module_id = _resolve_relative_import(module_id, import_)
                    # for import * indicate the module alias as an empty string
                    symbols[import_module_id] = _ImportAll()
