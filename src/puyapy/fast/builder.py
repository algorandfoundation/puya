...
_UNSUPPORTED_SYNTAX_MSG = "unsupported syntax"
_INVALID_PYTHON_MSG = "invalid Python"
...
def parse_module(
    *,
    ...
    try:
        ...
        logger.error(f"{_INVALID_PYTHON_MSG}: {e.msg}", location=loc)  # noqa: TRY400
        fast = None
    except Exception:
...
class _BuildContext:
    ...
    fail_unsupported: bool = attrs.field(default=True, on_setattr=attrs.setters.frozen)
    ...
    def invalid(
        ...
        if detail is None:
            message = _INVALID_PYTHON_MSG
        else:
            message = f"{_INVALID_PYTHON_MSG}: {detail}"
        if not isinstance(loc, SourceLocation | None):
            loc = self.loc(loc)
        logger.error(message, location=loc)
        self._failures += 1

    def unsupported(self, loc: SourceLocation | _ASTNodeWithLocation | None, message: str) -> None:
        if not isinstance(loc, SourceLocation | None):
            loc = self.loc(loc)
        if self.fail_unsupported:
            ...
            logger.debug("<fast> " + message, location=loc)
...
def _convert_module(
    ...
    *,
    ...
    ctx = _BuildContext(
        ...
        fail_unsupported=False,  # TODO: add PuyaPyOptions.new_parser
    )
                    ...
                    for as_name in ast_stmt.names:
                        if as_name.name == "TYPE_CHECKING":
                            if as_name.asname in (None, "TYPE_CHECKING"):
                                ...
                                self._ctx.unsupported(
                                    as_name, "aliasing TYPE_CHECKING is not supported"
                                )
        return None
...
class _DunderAllTracker:
    ...
    def _extract_literal_str_list(ctx: _BuildContext, seq: ast.expr) -> list[str]:
        ...
        if not isinstance(seq, ast.List | ast.Tuple):
            ctx.unsupported(seq, "expected list literal or tuple literal")
        else:
            for el in seq.elts:
                match el:
                    case ast.Constant(value=str(name)):
                        result.append(name)
                    case _:
                        ctx.unsupported(el, "expected a string literal")
        return result
...
class _UnsupportedSyntaxError(CodeError):
    pass
...
def _visit_stmt(ctx: _BuildContext, node: ast.stmt) -> nodes.Statement | None:
    ...
    try:
        ...
        handler = _unsupported_stmt_type
    result = None
    try:
        result = handler(ctx, node)
    except _UnsupportedSyntaxError as ex:
        ctx.unsupported(ex.location or node, ex.msg)
    return result
...
def _visit_function_def(ctx: _BuildContext, func_def: ast.FunctionDef) -> nodes.FunctionDef:
    ...
    if ctx.scope_kind == "func":
        ctx.unsupported(loc, "nested functions are not supported")
    if func_def.type_comment is not None:
        ctx.unsupported(loc, _NO_TYPE_COMMENTS_MSG)
    type_params = getattr(func_def, "type_params", None)
    if type_params:
        ctx.unsupported(loc, _NO_TYPE_PARAMS_MSG)
...
def _convert_decorator_list(
    ...
    for decorator in decorator_list:
        ...
        if callee_name is None:
            ...
            ctx.unsupported(loc, "only direct function references are supported as decorators")
        else:
            result.append(
                nodes.Decorator(
                    callee=callee_name,
...
def _convert_arguments(ctx: _BuildContext, args: ast.arguments) -> tuple[nodes.Parameter, ...]:
    ...
    if args.vararg is not None:
        ctx.unsupported(args.vararg, "variadic arguments are not supported in user functions")
    ...
    if args.kwarg is not None:
        ctx.unsupported(args.kwarg, "variadic arguments are not supported in user functions")
...
def _visit_class_def(ctx: _BuildContext, class_def: ast.ClassDef) -> nodes.ClassDef:
    ...
    if ctx.scope_kind != "module":
        ctx.unsupported(loc, "nested classes are not supported")
...
def _convert_alias(ctx: _BuildContext, alias: ast.alias) -> nodes.ImportAs:
    ...
    if alias.name == "__all__" or alias.asname == "__all__":
        ctx.unsupported(loc, "importing __all__ is not supported")
...
def _convert_import_from(
    ...
    match node.names:
        case [ast.alias("*", None) as star]:
            if ctx.scope_kind != "module":
                ctx.invalid(star, "import * only allowed at module level")
            names = None
        case _:
            names = [_convert_alias(ctx, alias) for alias in node.names]
    loc = ctx.loc(node)
...
def _correct_relative_import(
    ...
    *,
    ...
    if len(parts) < relative:
        ctx.invalid(import_loc, "attempted relative import with no known parent package")
    if relative == 0:
        base = current_module
    else:
...
def _visit_keywords_list(
    ...
    for kw in keywords:
        if kw.arg is None:
            ...
            ctx.unsupported(kw, "keyword argument unpacking is not supported")
        elif kw.arg in kwargs:
            ctx.invalid(kw, f"keyword argument repeated: {kw.arg}")
        else:
...
def _visit_match_pattern(ctx: _BuildContext, pattern: ast.pattern) -> nodes.MatchPattern:
    ...
    match pattern:
        ...
        case _:
            raise _UnsupportedSyntaxError("unsupported match pattern type", loc)
...
def _unsupported_stmt_type(ctx: _BuildContext, stmt: ast.stmt) -> None:
    ctx.unsupported(stmt, "unsupported statement type")
...
_STATEMENT_HANDLERS: typing.Final[
    ...
    (
        ...
        (ast.AsyncFunctionDef, _unsupported_stmt_type),
        ...
        (ast.TypeAlias, _unsupported_stmt_type),
        ...
        (ast.AsyncFor, _unsupported_stmt_type),
        _stmt_visitor_pair(ast.While, _visit_while_loop),
        _stmt_visitor_pair(ast.If, _visit_if),
        (ast.With, _unsupported_stmt_type),
        (ast.AsyncWith, _unsupported_stmt_type),
        _stmt_visitor_pair(ast.Match, _visit_match),
        (ast.Raise, _unsupported_stmt_type),
        (ast.Try, _unsupported_stmt_type),
        (ast.TryStar, _unsupported_stmt_type),
        ...
        (ast.Global, _unsupported_stmt_type),
        (ast.Nonlocal, _unsupported_stmt_type),
...
def _unsupported_expr_type(ctx: _BuildContext, expr: ast.expr) -> typing.Never:
    loc = ctx.loc(expr)
    raise _UnsupportedSyntaxError("unsupported expression type", loc)
