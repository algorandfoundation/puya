...
from puyapy.models import ARC4MethodData, ContractFragmentBase
...
class FunctionASTConverter(BaseMyPyVisitor[Statement | Sequence[Statement] | None, NodeBuilder]):
    ...
    def _handle_proxy_assignment(
        ...
        decl = self.contract_method_info.fragment.resolve_state(member_name)
        if decl is None:
        ...
        decl.description = rvalue.args.description

        key = rvalue.args.key
        if key is None:
            key = BytesConstant(
                value=member_name.encode("utf8"),
                wtype=decl.typ.wtype,
                encoding=BytesEncoding.utf8,
                source_location=member_loc,
            )
        elif not isinstance(key, BytesConstant):
            logger.error(
                f"assigning {rvalue.pytype} to a member variable"
        ...
        match expr:
            case mypy.nodes.RefExpr(node=mypy.nodes.TypeInfo()) if (
                ...
                if isinstance(py_typ, pytypes.ContractType):
                    return ContractTypeExpressionBuilder(self.context, py_typ, fragment, expr_loc)
                if pytypes.ARC4ClientBaseType in py_typ.bases:  # provides type info only
                    return ARC4ClientTypeBuilder(self.context, py_typ, expr_loc, fragment)
