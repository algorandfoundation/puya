...
from puya.awst.nodes import AppStorageDefinition, BytesEncoding
...
class _ContractFragment(_UserContractBase):
    ...
    _state_defs: dict[str, AppStorageDeclaration] = attrs.field(factory=dict, init=False)
    ...
    def resolve_state(
        ...
        if include_inherited:
            for fragment in self.mro:
                result = fragment.resolve_state(name, include_inherited=False)
                if result is not None:
                    return result
        return None

    def add_state(self, decl: AppStorageDeclaration) -> None:
        existing = self.resolve_state(decl.member_name)
    ...
    @property
    def reserved_scratch_space(self) -> StableSet[int]:
