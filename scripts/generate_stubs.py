...
from puyapy.awst_build.intrinsic_models import FunctionOpMapping
...
class RenamedOpCode(OpCodeGroup):
    name: str
    op: str
    doc_aliases: Sequence[str] = attrs.field(default=())
    """related op codes listed alongside `op` in stub docstrings only"""

    def handled_ops(self) -> Iterator[str]:
        yield self.op
        yield from self.doc_aliases
...
class MergedOpCodes(OpCodeGroup):
    ...
    """outer op -> related op codes for stub docstrings only"""

    def handled_ops(self) -> Iterator[str]:
        for op, doc_aliases in self.ops.items():
            yield op
            yield from doc_aliases
...
RENAMED_OP_CODES = [
    ...
    RenamedOpCode(
        name="replace",
        op="replace3",
        doc_aliases=["replace2"],
    ),
    RenamedOpCode(
        name="substring",
        op="substring3",
        doc_aliases=["substring"],
    ),
    RenamedOpCode(
        name="gload",
        op="gloadss",
        doc_aliases=["gload", "gloads"],
    ),
    RenamedOpCode(
        name="gaid",
        op="gaids",
...
class FunctionDef:
    ...
    op_mapping: FunctionOpMapping
    teal_op_codes: list[str]
    min_avm_version: int
...
def result_type(type_name: StackType) -> pytypes.RuntimeType:
    # the concrete pytype returned from ops with this stack type
    assert type_name is not StackType.any
    pytype = _stack_type_to_pytypes(type_name)[0]
    assert isinstance(pytype, pytypes.RuntimeType)
    return pytype
...
def build_method_stub(function: FunctionDef, prefix: str = "") -> Iterable[str]:
    ...
    returns = pytype_stub_repr(function.op_mapping.result_pytype)
    if return_docs:
        if doc:
    ...
    teal_op_desc = ", ".join(_get_algorand_doc(teal_op) for teal_op in function.teal_op_codes)
    doc.append("")
...
def build_class_from_overriding_immediate(
    ...
    # copy input so it can be mutated safely
    op = copy.deepcopy(op)

    stack_to_modify = _get_modified_stack_value(op)
    ...
    for value in arg_enum_values:
        ...
        method = build_operation_method(
            op,
            _snake_case(value.name),
            doc_aliases=doc_aliases,
            const_immediate_value=(immediate, value),
        )
        ...
        class_ops.update(method.teal_op_codes)
...
def build_operation_method(
    ...
    *,
    ...
    for idx, i_arg in enumerate(op.immediate_args):
    ...
    proto_function = FunctionDef(
        ...
        op_mapping=func,
        teal_op_codes=sorted({op.name, *doc_aliases}),
...
def build_operation_methods(
    op: Op, op_function_name: str, *, doc_aliases: Sequence[str] = ()
    ...
    if StackType.any in (s.stack_type for s in op.stack_outputs):
        ...
        for replace_any_with in (StackType.bytes, StackType.uint64):
            ...
            yield build_operation_method(new_op, new_name, doc_aliases=doc_aliases)
    else:
        yield build_operation_method(op, op_function_name, doc_aliases=doc_aliases)
...
def build_aliased_ops(spec: LanguageSpec, group: RenamedOpCode) -> Iterable[FunctionDef]:
    ...
    methods = build_operation_methods(op, group.name, doc_aliases=group.doc_aliases)
    return methods
...
def build_merged_ops(spec: LanguageSpec, group: MergedOpCodes) -> ClassDef:
    ...
    for other_op_name, doc_aliases in group.ops.items():
...
def build_grouped_ops(
    ...
    for rename_op_name, python_name in group.ops.items():
        ...
        if rename_immediate:
            ...
            methods.extend(build_operation_methods(rename_op, python_name))
...
def build_op_specification_body(function: FunctionDef) -> Iterable[str]:
    op_mapping = function.op_mapping
    if function.is_property:
        (immediate,) = op_mapping.immediates
        yield (
            f"{function.name}=PropertyOpMapping("
            f"{op_mapping.op_code!r}, {immediate!r}, {pytype_repr(op_mapping.result_pytype)},"
            f"),"
        )
    else:
        yield f"{function.name}=FunctionOpMapping(op_code={op_mapping.op_code!r},"
        if op_mapping.result is None:
            yield " result=None,"
        elif op_mapping.result:
            yield f" result=[{', '.join(pytype_repr(r) for r in op_mapping.result)}],"
...
def build_awst_data(
    ...
    yield autogenerated_header_for(__file__)
    yield "import typing"
    yield "from collections.abc import Mapping, Sequence"
    yield "from puyapy.awst_build import pytypes"
    yield "from puyapy.awst_build.intrinsic_models import FunctionOpMapping, PropertyOpMapping"
    ...
    yield ")"
    yield ""
    yield "FUNC_TO_AST_MAPPER: typing.Final[Mapping[str, FunctionOpMapping]] = dict("
