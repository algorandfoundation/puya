...
_STATIC_STRUCT = "(uint64,uint8)"
_DYNAMIC_STRUCT = "(uint64,uint8,(len+utf8[]))"
_WITH_A_BOOL = "(uint8,(len+uint8[]),bool1)"
...
def _invalid_arr_size(element_type_name: str, size: int | None = None) -> str:
    if size is None:
        return _invalid_size(f"(len+{element_type_name}[])")
    else:
        return _invalid_size(f"{element_type_name}[{size}]")


@pytest.mark.parametrize(
    ("type_name", "size_or_bytes_value", "expected_error"),
    [
        ("uint64", 7, _invalid_size("uint64")),
        ("ufixed64", 7, _invalid_size("uint64")),
        ("uint8", 2, _invalid_size("uint8")),
        ("bool", 2, _invalid_size("bool")),
        ("byte", 2, _invalid_size("uint8")),
        ("string", 5, _invalid_arr_size("utf8")),
        ("bytes", 5, _invalid_arr_size("uint8")),
        ("address", 0, _invalid_arr_size("uint8", 32)),
        ("address", 33, _invalid_arr_size("uint8", 32)),
        ("account", 0, _invalid_size("uint8")),
        ("account", 33, _invalid_size("uint8")),
        ("uint512", 63, _invalid_size("uint512")),
        ("uint8_arr", 1, "invalid array length header"),
        ("uint8_arr", 3, _invalid_arr_size("uint8")),
        ("uint8_arr3", 1, _invalid_arr_size("uint8", 3)),
        ("uint8_arr3", 4, _invalid_arr_size("uint8", 3)),
        ("bool_arr", b"\x00\x01", _invalid_arr_size("bool1")),
        ("bool_arr", b"\x00\x09\x00", _invalid_arr_size("bool1")),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "dynamic_struct_imm_arr",
            29,
            _invalid_arr_size(_DYNAMIC_STRUCT),
        ),
