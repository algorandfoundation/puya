...
# TODO: test other code fix types
async def test_code_fix_suggested(unique_uri: str, harness: "_LanguageServerHarness") -> None:
    ...
    assert [e.new_text for e in code_action.edit.changes[unique_uri]] == [
...
async def test_puya_is_shutdown_when_ls_stops(workspace_root: Path, unique_uri: str) -> None:
    import psutil

    # create an instance of language server
    client = await _create_client()
    harness = _LanguageServerHarness(client, workspace_root)
    process = client.server
    puyapy_lsp = psutil.Process(process.pid)

    # check no child process initially
    assert puyapy_lsp.is_running(), "expected language server to be running"
    assert puyapy_lsp.children() == [], "expected no children before initialization"
    await harness.initialize()
    harness.open_doc(uri=unique_uri, text="""import algopy""")
    await harness.wait_for_diagnostic(unique_uri, 1)

    # puya process should be present after diagnostic has been received
    children = puyapy_lsp.children()
    assert len(children) == 1, "expected 1 child after initialization"
    (child,) = children
    child_info = child.as_dict()
    assert child_info["cmdline"][-2:] == ["puya", "serve"]
    assert child.is_running(), "expected puya to be running"

    # check puya is shutdown during client shutdown
    await client.shutdown_async(None)
    assert not child.is_running(), "expected puya to have stopped"
...
def unique_uri(workspace_root: Path) -> str:
    ...
    unique_path = workspace_root / f"{name}.py"
...
class _LanguageServerHarness:
    ...
    def __init__(self, client: LanguageClient, workspace_root: Path) -> None:
        self.client = client
    ...
    def open_doc(self, uri: str, text: str) -> None:
        # create the actual file on disk to so mypy's module discovery continues to work
        path_str = to_fs_path(uri)
