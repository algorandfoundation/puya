from puyapy import Contract, Transaction, arc4
...
class MyContract(Contract):
    def approval_program(self) -> bool:
        ...
        # Account is valid
        sender_address = arc4.Address(Transaction.sender())
        # When creating an address from bytes, we check the length is 32 as we don't know the
        # source of the bytes
        checked_address = arc4.Address(Transaction.sender().bytes)
        # When using from_bytes, no validation is performed as per all implementations of
        # from_bytes
        unchecked_address = arc4.Address.from_bytes(Transaction.sender().bytes)
