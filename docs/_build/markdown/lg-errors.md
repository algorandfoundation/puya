...
the [`op.err()`](api-algopy.op.md#algopy.op.err) operation. This operation causes the TEAL program to immediately
and unconditionally fail.

Alternatively [`op.exit(0)`](api-algopy.op.md#algopy.op.exit) will achieve the same result. A non-zero value will
...
For more details see [Unsupported Python features](lg-unsupported-python-features.md#raise-try-except-finally).
