---
title: Types
description: AVM types, Python built-in types, ARC-4 types, and template variables in Algorand Python
---
...
> [!NOTE]
> See [Python builtins](/puya/language-guide/builtins/#len) for an explanation of why `len()` isn't supported.
...
The `int`, `str` and `bytes` built-in types are currently only supported as [module-level constants](/puya/language-guide/modules/) or literals.
