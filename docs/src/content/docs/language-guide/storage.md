---
title: Storage
description: Global state, local state, boxes, and storage management in Algorand Python
---
...
the current method via [local variables and subroutine params](/puya/language-guide/structure/#subroutines).
...
:::note
Contracts using `GlobalMap` must specify adequate [`state_totals`](/puya/language-guide/structure/#contract-class-configuration) to
...
[`LocalMap`](/puya/api/algopy/algopy/#class-localmap) is similar to `LocalState`, but allows for grouping a set of local state values with a common key and content type.
A custom `key_prefix` can optionally be provided, with the default being to use the member variable name as the prefix.
The final state key is the combination of `key_prefix + key`. All access requires both an account reference and a key.

:::note
Contracts using `LocalMap` must specify adequate [`state_totals`](/puya/language-guide/structure/#contract-class-configuration) to
...
If none of these abstractions suit your needs, you can use the box storage [AVM ops](/puya/language-guide/ops/) to interact with box storage. These ops match closely to the opcodes available on the AVM.
...
To use scratch storage you need to [register the scratch storage that you want to use](/puya/language-guide/structure/#contract-class-configuration) and then you can use the scratch storage [AVM ops](/puya/language-guide/ops/).
