...
-   Requires [`.copy()`](#algopy.Struct.copy) when mutable and creating additional references
...
-   Requires [`.copy()`](#algopy.FixedArray.copy) if making additional references to the array or any mutable elements

### `algopy.Array`

-   Can contain any type except transactions
-   Dynamically sized, efficient for reading (when assembled off-chain). Inefficient to manipulate on-chain
-   Requires [`.copy()`](#algopy.Array.copy) if making additional references to the array or any mutable elements
...
-   An immutable copy can be made for storage or returning from a contract by using the [`freeze`](#algopy.ReferenceArray.freeze) method e.g.
...
-   Use [`algopy.ReferenceArray.freeze`](#algopy.ReferenceArray.freeze) to convert the array to an `algopy.ImmutableArray` for storage