...
[ARC32](https://github.com/algorandfoundation/ARCs/blob/main/ARCs/arc-0032.md) extends the concepts in ARC-4 to include an Application Specification which more holistically describes a smart contract and its associated state.
...
but a NamedTuple can be used in ABI methods are will be encoded/decode to an ARC-4 struct automatically.
...
To ensure semantic compatability the compiler will also check for any usages of mutable ARC-4 types (arrays and structs) and ensure that any additional references are copied using the `.copy()` method.
