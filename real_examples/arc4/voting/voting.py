...
from algopy.arc4 import DynamicArray, UInt8

VoteIndexArray: typing.TypeAlias = DynamicArray[UInt8]
...
class VotingContract(ARC4Contract):
    def __init__(self) -> None:
        ...
        self.snapshot_public_key: Bytes = empty_bytes
        self.vote_id: arc4.String = arc4.String.from_bytes(empty_bytes)
        self.metadata_ipfs_cid: arc4.String = arc4.String.from_bytes(empty_bytes)
        self.start_time: UInt64 = UInt64(0)
        self.end_time: UInt64 = UInt64(0)
        self.quorum: UInt64 = UInt64(0)
        self.nft_image_url: arc4.String = arc4.String.from_bytes(empty_bytes)
        ##self.option_counts: VoteIndexArray = DynamicArray[UInt8].from_bytes(empty_bytes)
        self.option_counts: VoteIndexArray = VoteIndexArray.from_bytes(empty_bytes)
        self.total_options: UInt64 = UInt64(0)

    def clear_state_program(self) -> bool:
        return True

    @arc4.abimethod(create=True, allow_actions=["no_op"])
    def create(
        self,
        vote_id: arc4.String,
    ...
    @arc4.abimethod()
    def bootstrap(self, fund_min_bal_req: arc4.PaymentTransaction) -> None:
        ...
        Box.create(TALLY_BOX_KEY, self.total_options)
    ...
    def close(self) -> None:
        ...
        note = (
            b'{"standard":"arc69","description":"This is a voting result NFT for voting round with ID '
            + self.vote_id.decode()
            + b'.","properties":{"metadata":"ipfs://'
            + self.metadata_ipfs_cid.decode()
            + b'","id":"'
            + self.vote_id.decode()
            + b'","quorum":'
            + itob(self.quorum)
        ...
        CreateInnerTransaction.set_config_asset_name(b"[VOTE RESULT] " + self.vote_id.decode())
        CreateInnerTransaction.set_config_asset_unit_name(b"VOTERSLT")
        CreateInnerTransaction.set_config_asset_url(self.nft_image_url.decode())
        CreateInnerTransaction.set_note(note)
        CreateInnerTransaction.submit()
        self.nft_asset_id = InnerTransaction.created_asset_id()

    @arc4.abimethod()
    def get_preconditions(self, signature: Bytes) -> tuple[bool, bool, bool, UInt64]:
        return (
            self.voting_open(),
            self.allowed_to_vote(signature),
            self.already_voted(),
    ...
    def vote(
        self,
        fund_min_bal_req: arc4.PaymentTransaction,
        signature: Bytes,
...
def increment_vote_in_box(index: UInt64) -> None:
    ...
    Box.replace(box_data, index, itob(current_vote + 1))
